<?php
  ...
  if (isset($_FILES["curriculo"])) {
    ...
    if (move_uploaded_file($_FILES["curriculo"]["tmp_name"], $target_file)) {
        ...
        $sql = "INSERT INTO agente (nome, email, telefone, tipo_vaga, sobre_mim, formacao_academica,experiencia_proficional, caminho_foto, curriculo_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
<head>
    ...
    <link rel="stylesheet" href="estilo/style_monitor.css">
    ...
    <?php
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td><a href='alterar_agente.php?id=" . $row["id"] . "'><button class='button_crud'>Editar</button></a></td>";
                echo "<td><button onclick='confirmDelete(" . $row["id"] . ")' class='button_crud delete'>Excluir</button></td>";
                echo "</tr>";
            }
        }
     ?>
